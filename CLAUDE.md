...
**Publishing also needs named sign-offs (0126).** `system_revision_approvals` is who agreed to a revision and when: one row per person, `required` (publish waits for them) or optional (a log entry). `publish_system_revision` raises if a revision has *no* approvers at all, or if any required one has a null `approved_at` — so a revision cannot publish until someone records who approved the procedure. Sign-offs can be entered by hand (the datetime is editable, not fixed) because they often record an agreement that happened elsewhere — your own goes in with one click, see below, and the **sign-offs** do not carry forward: a new revision is a new snapshot and needs its own agreement. The **names** do — the Send-for-review dialog pre-fills from the last revision that named anyone, walking back through the history, so the same reviewers aren't re-picked every time. Carry the people, never the `approved_at`. Anyone authenticated may write the rows; the admin/owner gate stays on the publish itself.

**The Approve button is one control doing three jobs** (`ApproveButton` in `SystemDetail.tsx`), because "approve" means different things to whoever is looking at the revision. If you are a named approver who hasn't signed, it records *your* sign-off and nothing else; nobody waits for a co-approver before they can click, since the sign-offs are independent and only the last required one flips the revision to Approved. The label follows the act: it reads **Sign off** while someone else is still outstanding and **Approve** only on the click that actually flips the revision, because a button saying "Approve" that moves nothing reads as a failure. If everyone required has signed and you may publish, it publishes. Otherwise it is disabled, naming who is outstanding. It renders for a named approver of **any** role — recording your own sign-off is not an admin act — but the publish leg stays gated on admin/owner, so a staff approver whose signature completes the set gets "an admin can now approve it" rather than a failed RPC. The hand-entered datetime rows stay for back-dating and for the shared `team@` login, where `currentUserId` is null and there is no "my" sign-off to record.
