...
**The month view is one component, two audiences.** `MonthCalendar` (`components/review/`) renders `monthGrid` from `src/lib/calendar-month.ts` — pure, Monday-first, and **string-keyed on local "YYYY-MM-DD" throughout**: `due_date` is a Postgres `date` with no timezone, and a `toISOString()` round trip moves every SAST date before 02:00 into the previous square. The grammar is three marks — an event is a filled band, a due item is an outlined chip, and only a late one carries the error colour, because a month with a normal amount of work in it must not read as an emergency. The client's calendar plots their own dated items; the staff tab adds briefed ClickUp tasks (`original_due_date`, `court !== 'done'`) and a client name per chip when no client is picked. Parked and settled rows are off both — a calendar is what is coming. Events are also filtered out of the staff **Sign-offs & asks** table for the same reason parked is: that tab is the queue people work through, and a row nobody can act on is not part of that work. The two are asymmetric on purpose: the token payload carries no ClickUp task dates and widening it to carry them would put staff-side data on a client's page.
