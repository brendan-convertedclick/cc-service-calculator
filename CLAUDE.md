...
Hostnames, and they are not the same thing:

| URL | What it is | How code gets there |
| --- | --- | --- |
| `https://conductor.stitch.net.za` | **Production.** Cloudflare Pages project `conductor` (direct upload, no Git integration), account "Converted Clicks Account". `stitch.net.za` (zone `cc77a4ce…`, same account) is the domain the agency's applications are collecting under — Conductor is not a Converted Click-only tool any more, it runs The Media Mixology's work too. | `npm run deploy` — builds and uploads `dist/`. Nothing deploys on push; a commit that is not deployed is not live. |
| `https://conductor.convertedclick.co.za` | **Production, the old address.** Same Pages project, same deployment. It is kept alive DELIBERATELY and must not be retired: every client sign-off link already emailed carries it (`reviewUrlFor` builds from `window.location.origin`), as does every ClickUp comment the edge functions have ever posted. Also reachable at `conductor-ehv.pages.dev`. | Nothing — it is a second custom domain on the same project. |
...
- Edge functions link users to prod, which is correct — those emails and ClickUp comments should not point at a laptop. **`APP_URL` is ONE constant, in `supabase/functions/_shared/helpers.ts`.** Seven functions each carried their own copy of the string until the stitch.net.za move, which meant finding all seven; do not reintroduce a local one. The matching default for the MCP server is `mcp-server/src/lookup.ts` (overridable with `CONDUCTOR_APP_URL`).
- **Changing `APP_URL` is only real once those seven functions are redeployed**, and pointing it at a hostname before that hostname resolves puts a dead link in every notification posted in between. New host first, deploy second.
...
A procedure with no revision row at all reads as Draft: nothing has been sent to anyone yet. `changes_requested` (0137) used to be plain `draft`, which made a revision someone had reviewed and left notes on indistinguishable from one nobody had opened. It carries no decliner/date of its own — the notes in `StepNotesPanel` are the substance, and the row still shows who sent it for review. On the RLS side it sits with draft/proposed: anyone authenticated may write it, only `published`/`superseded` are admin/owner.

**Anything can come back to Draft, and any role can bring it (0147).** In review, Requested changes and *Approved* all reopen through `system_revision_back_to_draft` — `BackToDraftButton`, in the Tasks-pane header and the Revisions card header. **It takes the whole procedure, not the revision it is handed (0158).** A system can hold more than one open revision — an undecided proposal still sitting under a later published one — so the RPC resolves the *system* from the argument and drafts every revision of it in `proposed`/`changes_requested`/`published` at once. Per-row it was a lie twice over: the procedure carried on reading "In review" in the list after somebody had deliberately un-approved it, the stale snapshot was still publishable back into force, and a system whose newest revision was already a draft could not be reopened from the Tasks pane at all (the button read that one row, saw `draft` and hid). That is also why the control is one button in a header rather than one per card — four buttons that all do the same thing to all four rows is a trap. This replaced the older rule that a revision never re-opens, so do not restore it. That rule existed to protect one thing: `system_revision_approvals` hang off a revision id, so reopening a signed row would let edited content ride on sign-offs recorded against a different snapshot. **The invariant is kept by the door rather than by the lock** — going back to Draft clears every `approved_at` on the way, which is the same rule the Send-for-review dialog already follows: carry the people, never the `approved_at`. The approver rows survive so the names still pre-fill.

Three things about it are load-bearing:
  * **It is an RPC, not a wider RLS policy.** `system_revisions_edit` refuses staff any UPDATE on a `published` row and must keep refusing: loosening it so they could set `state='draft'` would also let them rewrite published *content* in place, and a published revision that can be quietly edited is not evidence of anything. SECURITY DEFINER grants this one transition and nothing else — the mirror of `publish_system_revision` going the other way.
  * **No role check, deliberately.** Publishing is the one admin act; taking something back to be worked on is the opposite of an approval, and the person who finds a procedure wrong is the one running it. This does mean a staff member can un-approve a live procedure, and take every open revision of it back in the same click — restricting the `published` case is one added condition in the function.
  * **Un-publishing leaves nothing approved.** `current_revision_id` is nulled unconditionally — whatever it pointed at is in the set — and the superseded predecessor is **not** resurrected: `publish_system_revision` requires `proposed`, so there is no legal path back for that row, and silently promoting an older version would change what the team is told to follow without anyone deciding to. `superseded` therefore cannot reopen at all — it is history, and reopening it would fork the procedure into two live versions of the past.

It posts nothing to the ⚙️ Systems channel. The three transitions that do are the ones another person is waiting on; taking your own work back to finish it is not news.

**Systems approvals post to ClickUp.** `notify-system-revision` puts all three *forward* transitions — sent for review, approved, changes requested — into the **⚙️ Systems** ops channel (`SYSTEMS_CHANNEL_ID` in `_shared/clickup-chat.ts`), the fourth of the workspace-wide ops channels alongside 🆕 New Tasks / 📅 Meetings / ✅ Approval Requests. "Sent for review" pings the revision's **required** approvers by name (falling back to every admin and owner for revisions proposed before the dialog collected them); approved and changes-requested ping the proposer and the system's owner, deduped. It is fired from `onSuccess` in `useSystemRevisions.ts` — never from a `mutationFn`, so a chat outage can't fail a publish — and every call is `.catch(() => {})`, which means a bad deploy is **silent**: it must be deployed `--no-verify-jwt` like every other function here, and blanking the channel constant makes it no-op rather than error.
...
**The list runs both ways now (0149).** A client holding a link could only ever respond — answer, decide, reply. Two things they most want to *start* had nowhere to go, so they went to email, where the list cannot see them and nobody is counted as waiting. `action: "raise"` on `client-review` takes exactly two kinds and no more:

  * **A question they ask us** reuses `item_type = 'question'` with `owed_by = 'us'` — the shape already existed, only the direction is new, which is why `client_approvals_owed_by_type_chk` widens by exactly one value. It lands in **With us** with no controls: a question you asked is not a question you answer. `typeLabelFor` reads "You asked", `threadOf` opens the thread with **their** bubble in their name, and the settled line says "Answered — it's above" rather than "we have your answer".
  * **An event** is a date in their world — a launch, a sale, an office move. It is the only row on this table nobody acts on, which is exactly why it gets its own **state**, `noted`, rather than a type exclusion bolted onto every count. Every clock and control here is written as `state = 'pending'`; a pending event would be silently wrong in all of them (the staff table would call a launch date "3d late", `handleDecide`'s guard would accept an Approve on it, `daysWaiting` would flag it red). With a distinct state all of that is correct by default and exactly three places know about events on purpose: `bucketOf` → **Coming up**, `countStages` skips them, and `sortForQueue` reads them as a diary — soonest first, not by title. `client_approvals_event_state_chk` locks the pairing both ways, so an event can never be settled and a wrong date is a delete, not a state change.

`raised_by` / `raised_by_name` carry who started it. The name is resolved **server-side from the token contact** and snapshotted like `decided_by_name` (0142) — nothing in the request body can name anyone, which is the same rule a decision follows. A legacy shared link has nobody to name and is recorded anonymously rather than refused. `eventDateLabel` is the one place an event's date becomes text; `dueStatus` deliberately returns null for it, because a date passing is not lateness.

**The month view is one component, two audiences.** `MonthCalendar` (`components/review/`) renders `monthGrid` from `src/lib/calendar-month.ts` — pure, Monday-first, and **string-keyed on local "YYYY-MM-DD" throughout**: `due_date` is a Postgres `date` with no timezone, and a `toISOString()` round trip moves every SAST date before 02:00 into the previous square. The grammar is four marks — an event is a filled band, a due item is an outlined chip, a done one is that chip faded and ticked, and only a late one carries the error colour, because a month with a normal amount of work in it must not read as an emergency. `calendarEntriesFor` in `client-review.ts` is the one mapping and **both** calendars go through it, so staff and client cannot be shown two different Marches; the staff tab then adds briefed ClickUp tasks (`original_due_date`, `court !== 'done'`) and a client name per chip when no client is picked. Parked rows are off both, dropped inside the mapping — a parked item is on nobody's clock and a calendar is a clock. Events are also filtered out of the staff **Sign-offs & asks** table for the same reason: that tab is the queue people work through, and a row nobody can act on is not part of that work.

**A finished thing sits on the day it finished (0159).** Settled work used to be left off the calendar entirely, on the argument that a month full of things already dealt with buries the two that are not. That was right about the *position* and wrong about the omission: an approved sign-off now plots at its `decided_at`, not its due date, so it lands in the month it happened and last month reads as a record rather than a row of deadlines that have gone by. Undecided asks stay on their due date, which is the date that still matters. A settled row with no stamp at all is an old one and falls back to its due date rather than off the grid.

**A school sees its own year (0159).** `client_pipeline_schedule` is a read-only view of `school_tasks` and the client-review payload carries it as `schedule`, beside `items` — a plan to look at, not a queue to act on, so its chips are `pickable: false`. Three things make it safe and one makes it truthful:
  * **It is a view, and the view is what leaves the staff columns behind.** Assignee, department, estimate and who-moved-it are absent from it, so rule 1 of `types/client-review.ts` is enforced by the query rather than by every caller remembering. Never select the client's plan off `school_tasks` directly. This does not contradict the old "the token payload carries no ClickUp task dates" line — that was about the agency's briefed backlog; a school's own year is theirs by design, and the only ClickUp fact that crosses is when their own work was finished.
  * **Our work must not become `client_approvals`.** `school_tasks_approval_side_chk` forbids it, an `agreement` needs an `agreed_at`, and ninety-odd rows in a decision queue is a wall, not a plan. Rows that *have* become an ask are excluded from the view, or they would be on the calendar twice.
  * **A planned task has a square without having a date.** It has no `due_date` and must not gain one, but the date the month will hand it is knowable, so `school_task_due_on` is now the single home of that rule — the month end, or the six-week gate for the gate task of an `open_day_before` month — and `schedule_school_year_month` reads it instead of holding a second copy. Its `p_clamp_forward` parameter is the **write** rule ("no task is born late") and the calendar passes false: both live years were mapped retroactively from January, so a clamping calendar would pile eight months of plan onto today's square and move it every morning. `tg_school_tasks_guard` deliberately still does not use it — a task dragged into a month is not a task the month handed out.
  * **`completed_at` prefers ClickUp's own date** (`briefs.completed_at`, written from `date_done`/`date_closed` by `sync-clickup-actuals`) and falls back to the staff tick. **`school_tasks.brief_id` is still written by nothing**, so today that fallback is all there is — the ClickUp date starts flowing the moment a "brief it" exists on a pipeline task, with no change to this path.

**A fourth thing exists that is waiting on nobody (0148).** `state = 'parked'` is an item that is on the list and off every clock: no overdue, no chasing, no "waiting on this client" count. It joins `pending` on the undecided side of `client_approvals_decided_chk`, which is why parking clears the decision stamp exactly as reopening does — `useSetItemState` settles only on `approved`/`changes_requested`, and anything else is a move, not a decision. **Parked is staff-only**: `client-review`'s list query and the `useClientReviewPreview` mirror both `.neq("state", "parked")`, and so do its reply and decide lookups, so an item nobody is acting on is not something a client is shown or can act on. `fetchStageCounts` excludes them too — a parked row landing in any of the three email buckets would put a number in a chase that the client cannot see.

`item_type = 'idea'` is the thing that previously had no home at all: worth considering, not planned. It exists because the other three each carry a constraint that does not fit one — a brief needs a `brief_id`, a question emails the moment it is written, an agreement needs the date it was agreed. **An idea is always parked** (`client_approvals_idea_parked_chk`), and that constraint is load-bearing rather than tidy: `toReviewItem` degrades an unrecognised `item_type` to `brief` so no client ever meets an item with no way to act, so a *pending* idea would reach their page wearing Approve and Request-changes buttons. The database refuses to make one. An idea therefore has no **Unpark** control — it leaves the list by being asked properly as a question, an agreement or a brief, which is the act of deciding what it actually is. A parked *question* or *brief* does unpark, from the Parked tab. Ideas carry `owed_by` like agreements do (whose move it will be when the time comes), which is the one fact a thing with no clock still has.
...
- **Every prod hostname** must be listed in Supabase Auth → URL Configuration → Redirect URLs, or the provider refresh token is never captured on that host and every meeting reports "No Google account connected". That is now `conductor.stitch.net.za` as well as `conductor.convertedclick.co.za`; `conductor-dev.convertedclick.co.za` and `conductor-ehv.pages.dev` are on the allow list too, so a Google sign-in works on the dev preview as well. Adding a hostname to Cloudflare without adding it here is the failure that looks like "Google just stopped working".
