...
`ApproveButton` is the revision-level control that remains — the publish leg, disabled with the reason until everyone required has signed. The revision card passes `publishOnly` because its rows already offer the sign-off; the Steps-pane header has no such list, so there it keeps both legs. The card header also keeps **Request changes** only for an admin who is *not* a named approver — anyone named acts on their own line instead.
