...
export function ServiceDetail({ mode }: Props) {
  ...
  function onSave() {
    const payload = {
      ...
      percentage_value: form.pricing_model === "percentage" ? form.percentage_value : null,
  ...
  return (
    <div className="container mx-auto max-w-5xl p-6">
      ...
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{mode === "new" ? "New service" : form.name}</h1>
          {mode === "edit" && form.code && <p className="text-sm text-muted-foreground font-mono">Code {form.code}</p>}
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-[2fr_3fr]">
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  <Label>Status</Label>
                  <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="h-9 w-full rounded-md border bg-background px-2 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={form.unit_of_sale} onChange={(e) => setForm({ ...form, unit_of_sale: e.target.value })} placeholder="Per page, Per hour…" />
                </div>
              </div>
              {form.pricing_model === "percentage" ? (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, percentage_value: e.target.value === "" ? null : Number(e.target.value) })}
                  />
                  <p className="text-xs text-muted-foreground">Fee over spend. Hour allocation doesn't apply.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, sell_price_cents: Math.round(Number(e.target.value) * 100) })}
                  />
                  <p className="text-xs text-muted-foreground">{formatZar(form.sell_price_cents)}</p>
                </div>
              )}
              <div className="space-y-2">
                <Label>Rule</Label>
                <select
                  value={form.rule_id ?? ""}
                  onChange={(e) => setForm({ ...form, rule_id: e.target.value || null })}
                  className="h-9 w-full rounded-md border bg-background px-2 text-sm"
                >
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, primary_team_member_id: e.target.value || null })}
                  className="h-9 w-full rounded-md border bg-background px-2 text-sm"
                >
                  <option value="">—</option>
                  {team.map((m) => (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {(["scope_definition", "included_revisions", "trigger_to_start", "completion_definition", "notes"] as const).map((field) => (
